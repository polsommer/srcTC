...
ConnectionServer::ConnectionServer()
        : MessageDispatch::Receiver(), chatService(0), clientServicePrivate(0),
          clientServicePublic(0), gameService(0), loginServerKeys(0), done(false), m_id(0), m_metricsData(0),
          centralConnection(0), chatServers(), clientMap(), connectedMap(), gameServerMap(),
...
void ConnectionServer::handleConnectionServerIdMessage(const ConnectionServerId &m) {
                                                                   ...
                                                                   nullptr) //lint !e774 // always evaluates to false // suppresed because the FATAL macro triggers this lint warning
    {
        ...
        const NewCentralConnectionServer ncs(gameService->getBindAddress(), clientServicePublicBindAddress, chatService->getBindAddress(), std::string(), privatePort, publicPort, g->getBindPort(), chatPort, 0, pingPort, ConfigConnectionServer::getConnectionServerNumber());
...
void ConnectionServer::addToConnectedMap(uint32 suid, ClientConnection *cconn) {
    ...
    if (i == connectedMap.end()) {
        ...
        DEBUG_WARNING(true, ("Attempt made to log in an additional character from the same account"));
        connectedMap[suid] = cconn;
    }
