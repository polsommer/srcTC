...
namespace
{
    ...
    std::unique_ptr<Base> upcastUniquePtr(std::unique_ptr<Derived> &&ptr)
    {
        return std::unique_ptr<Base>(ptr.release());
    }

    template <typename Derived, typename Base>
    std::unique_ptr<Derived> downcastUniquePtr(std::unique_ptr<Base> &ptr)
    {
        if (auto derived = dynamic_cast<typename std::unique_ptr<Derived>::pointer>(ptr.get()))
        {
            ptr.release();
...
PlanetServer::PlanetServer() :
		...
		m_roundRobinGameServer(0),
                m_pendingServerStarts(),
                m_startingGameServers(),
                m_firstGameServer(0),
                m_tutorialMode(false),
                m_spaceMode(false),
                m_messagesWaitingForGameServer(),
                m_metricsData(nullptr),
                m_taskManagerConnection(nullptr),
                m_sceneTransferChunkLoads(),
                m_pendingCharacterSaves(),
                m_watchers(),
...
PlanetServer::~PlanetServer()
{
	...
	MetricsManager::remove();
        ...
        m_sceneTransferChunkLoads.clear();
        m_pendingCharacterSaves.clear();
...
void PlanetServer::receiveMessage(const MessageDispatch::Emitter & source, const MessageDispatch::MessageBase & message)
{
	...
	switch(messageType) {
		...
		{
                        Archive::ReadIterator ri = static_cast<const GameNetworkMessage &>(message).getByteStream().begin();
                        std::unique_ptr<const RequestGameServerForLoginMessage> msg(new RequestGameServerForLoginMessage(ri));

                        handleRequestGameServerForLoginMessage(std::move(msg)); // ownership transferred; msg is invalid after this call.
                        break;
                }
                case constcrc("RequestSceneTransfer") :
                {
                        ...
                        Archive::ReadIterator ri = static_cast<const GameNetworkMessage &>(message).getByteStream().begin();
                        std::unique_ptr<const RequestSceneTransfer> msg(new RequestSceneTransfer(ri));

                        handleRequestSceneTransfer(std::move(msg)); // ownership transferred; msg is invalid after this call.
                        break;
                }
		...
		case constcrc("WatcherConnectionOpened") :
		{
		...
		case constcrc("WatcherConnectionClosed") :
		{
		...
		case constcrc("RequestSameServer") :
		{
			Archive::ReadIterator ri = static_cast<const GameNetworkMessage &>(message).getByteStream().begin();
		...
		case constcrc("RequestAuthTransfer") :
		{
			Archive::ReadIterator ri = static_cast<const GameNetworkMessage &>(message).getByteStream().begin();
		...
		case constcrc("GameServerForceChangeAuthorityMessage") :
		{
			Archive::ReadIterator ri = static_cast<const GameNetworkMessage &>(message).getByteStream().begin();
		...
		case constcrc("FindAuthObjectResponse") :
		{
			Archive::ReadIterator ri = static_cast<const GameNetworkMessage &>(message).getByteStream().begin();
		...
		case constcrc("DenySetAuthoritativeMessage") :
		{
			Archive::ReadIterator ri = static_cast<const GameNetworkMessage &>(message).getByteStream().begin();
...
void PlanetServer::handleRequestGameServerForLoginMessage(std::unique_ptr<const RequestGameServerForLoginMessage> msg)
{
        NOT_NULL(msg.get());
        ...
        if (!PreloadManager::getInstance().isPreloadComplete())
        {
                ...
                m_messagesWaitingForGameServer.push_back(upcastUniquePtr<const GameNetworkMessage>(std::move(msg)));
                return;
        }
...
void PlanetServer::findOrLoadCharacter(NetworkId const &characterId, NetworkId const &containerId, Vector const &coords, uint32 stationId, bool forCtsSourceCharacter)
{
	// put in map along with expected response count
        ...
        {
                forceLoadCharacter(characterId, containerId, coords, stationId, forCtsSourceCharacter);
                return;
        }
...
void PlanetServer::forceLoadCharacter(NetworkId const &characterId, NetworkId const &containerId, Vector const &coords, uint32 stationId, bool forCtsSourceCharacter)
{
	...
	{
		DEBUG_REPORT_LOG(true, ("Waiting for game server for login\n"));
                std::unique_ptr<const RequestGameServerForLoginMessage> queuedMessage(new RequestGameServerForLoginMessage(
                                stationId,
                                characterId,
                                containerId,
                ...
                m_messagesWaitingForGameServer.push_back(upcastUniquePtr<const GameNetworkMessage>(std::move(queuedMessage)));
	}
	else
	{
		if (!node->isLoaded())
...
void PlanetServer::handleRequestSceneTransfer(std::unique_ptr<const RequestSceneTransfer> msg)
{
        NOT_NULL(msg.get());
        ...
        {
                node->load();
                m_sceneTransferChunkLoads.push_back(std::move(msg));
        }
        else
        {
                if (gameServerId == 0)
...
void PlanetServer::handleChunkComplete(const ChunkCompleteMessage &msg, const GameServerConnection *conn)
{
	...
	for (std::vector<std::pair<int, int> >::const_iterator chunk=chunks.begin(); chunk!=chunks.end(); ++chunk)
	{
		...
		int nodeZ = chunk->second;
                ...
                std::list<std::unique_ptr<const RequestSceneTransfer> >::iterator i = m_sceneTransferChunkLoads.begin();
                while (i != m_sceneTransferChunkLoads.end())
                {
                        const RequestSceneTransfer *transferMsg = i->get();
...
void PlanetServer::flushWaitingMessages()
{
	...
	DEBUG_REPORT_LOG(true,("Handling pending logins and transfers.\n"));
        std::vector<std::unique_ptr<const GameNetworkMessage> >::iterator t;
        for(t = m_messagesWaitingForGameServer.begin(); t != m_messagesWaitingForGameServer.end();)
        {
                std::unique_ptr<const GameNetworkMessage> &msg = *t;
                if (!msg)
                {
                ...
                if (std::unique_ptr<const RequestSceneTransfer> sceneTransfer = downcastUniquePtr<const RequestSceneTransfer>(msg))
                {
                        t = m_messagesWaitingForGameServer.erase(t);
                        handleRequestSceneTransfer(std::move(sceneTransfer));
                        continue;
                }

                if (std::unique_ptr<const RequestGameServerForLoginMessage> loginMessage = downcastUniquePtr<const RequestGameServerForLoginMessage>(msg))
                {
                        t = m_messagesWaitingForGameServer.erase(t);
                        handleRequestGameServerForLoginMessage(std::move(loginMessage));
                        continue;
                }
...
void PlanetServer::queueMessageForObject(const NetworkId &networkId, std::unique_ptr<GameNetworkMessage> theMessage)
{
        if (!theMessage)
                return;
        ...
        std::vector<std::unique_ptr<GameNetworkMessage> > & queuedMessages = m_queuedMessages[networkId];
        queuedMessages.push_back(std::move(theMessage));
}
...
void PlanetServer::sendQueuedMessagesForObject(const PlanetProxyObject &theObject)
{
        ...
        if (i!=m_queuedMessages.end())
        {
                ...
                std::vector<std::unique_ptr<GameNetworkMessage> > & queuedMessages = i->second;
                for (std::unique_ptr<GameNetworkMessage> &message : queuedMessages)
                {
                        if (message)
                                sendToGameServer(server, *message);
                }
