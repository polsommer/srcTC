...
void PlanetProxyObject::sendAddProxy(uint32 proxyServer)
{
	...
	{
                std::unique_ptr<GameNetworkMessage> loadMsg(new LoadObjectMessage(getObjectId(), proxyServer));
                PlanetServer::getInstance().queueMessageForObject(getObjectId(), std::move(loadMsg));
	}
}
...
void PlanetProxyObject::sendRemoveProxy(uint32 proxyServer)
{
	...
	{
                std::unique_ptr<GameNetworkMessage> msg(new UnloadProxyMessage(getObjectId(), proxyServer));
                PlanetServer::getInstance().queueMessageForObject(getObjectId(), std::move(msg));
	}
}
