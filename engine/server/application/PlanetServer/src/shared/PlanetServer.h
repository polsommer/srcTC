...
public:
	...
	int                  getCurrentServerInterestObjectLimit() const;
        void                 queueMessageForObject(const NetworkId &networkId, std::unique_ptr<GameNetworkMessage> theMessage);
...
private:
        void                        handleRequestGameServerForLoginMessage  (std::unique_ptr<const RequestGameServerForLoginMessage> msg);
	void                        loadCharacterForLogin(NetworkId const &characterId, Vector const &coords, unsigned long gameServerId);
        void                        handleRequestSceneTransfer(std::unique_ptr<const RequestSceneTransfer> msg);
...
private:
	...
	std::set<GameServerConnection *> m_pendingGameServerDisconnects;
        ...
        std::vector<std::unique_ptr<const GameNetworkMessage> > m_messagesWaitingForGameServer;
        PlanetServerMetricsData*                m_metricsData;
        TaskConnection*                         m_taskManagerConnection;
        std::list<std::unique_ptr<const RequestSceneTransfer> > m_sceneTransferChunkLoads;
        std::map<NetworkId, uint32>          m_pendingCharacterSaves;

	WatcherList m_watchers;
	bool m_watcherIsPresent;

        typedef std::map<NetworkId, std::vector<std::unique_ptr<GameNetworkMessage> > > QueuedMessagesType;
