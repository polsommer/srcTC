...
{
	bool gs_connectionServersPublic=true;
...
void CentralServer::addGameServer(GameServerConnection * newGameServer)
{
    ...
    if (newGameServer->getSceneId().empty())
    {
        WARNING_STRICT_FATAL(true, ("Refusing to add game server %lu with empty scene id", pid));
        newGameServer->disconnect();
        return;
    }
...
void CentralServer::launchStartingProcesses() const
{
	...
	// Ensure connection and required data are available before launching
	bool hasTaskManager = m_taskManager != nullptr;
	bool isConnected = hasTaskManager && m_taskManager->isConnected();
	bool hasClusterId = m_clusterId != 0;

	if (!hasTaskManager || !isConnected || !hasClusterId)
	{
		if (!hasTaskManager)
		{
		...
		if (!hasClusterId)
		{
			REPORT_LOG(true, ("CentralServer not launching starting processes because m_clusterId is 0 - not received from the LoginServer yet\n"));
		}
		return;
	}

	REPORT_LOG(true, ("CentralServer now launching starting processes because it has a connection to the TaskManager AND has received cluster Id (%u) from the LoginServer\n", m_clusterId));

	startingProcessesLaunched = true;

	// Launch connection servers
	launchStartingConnectionServers();

	// Helper lambda to get service bind address or fallback to hostname
	auto getAddress = [](const Service* service) -> std::string {
		return service ? service->getBindAddress() : NetworkHandler::getHostName();
	};

        // Launch SwgDatabaseServer
	std::string dbOptions = "-s dbProcess centralServerAddress=" + getAddress(getGameService());
	TaskSpawnProcess dbProcess(ConfigCentralServer::getDbServerHost(), "SwgDatabaseServer", dbOptions);
	m_taskManager->send(dbProcess, true);

	// Launch ChatServer
	std::string chatOptions = "-s ChatServer centralServerAddress=" + getAddress(getChatService());
	chatOptions += std::string(" clusterName=") + ConfigCentralServer::getClusterName();
	TaskSpawnProcess chatProcess(ConfigCentralServer::getChatServerHost(), "ChatServer", chatOptions);
	m_taskManager->send(chatProcess, true);
...
void CentralServer::receiveMessage(const MessageDispatch::Emitter & source, const MessageDispatch::MessageBase & message)
{
	...
	switch (messageType) {
		...
		case constcrc("TaskConnectionClosed") : {
			//		Net::getInstance().connect(Network::Address("192.168.88.165", ConfigCentralServer::getTaskManagerPort()), TaskConnection());
			break;
		}
...
void CentralServer::run(void)
{
	...
	cserver.m_taskManager = new TaskConnection("192.168.88.165", ConfigCentralServer::getTaskManagerPort());
