...
void CentralServerConnection::onReceive(const Archive::ByteStream & message)
{
    ...
    const uint32_t messageType = msg.getType();

    switch (messageType) {
        case constcrc("TransferRenameCharacter"): {
            // Extract TransferCharacterData from the message
            ...
            // Call Persister to rename the character
            Persister::getInstance().renameCharacter(
                0,
                static_cast<int8>(RenameCharacterMessageEx::RCMS_cts_rename),
                request.getValue().getSourceStationId(),
                request.getValue().getCharacterId(),
                Unicode::narrowToWide(request.getValue().getDestinationCharacterName()),
                Unicode::narrowToWide(request.getValue().getSourceCharacterName()),
                false,
        ...
        case constcrc("ConGenericMessage"): {
            // Handle Console generic message
        ...
        case constcrc("TransferAccountRequestCentralDatabase"): {
            ...
            // Call Persister to change station ID
            Persister::getInstance().changeStationId(&requestData);
            break;
        }
        default: {
            // Handle unknown message types by passing them to the base class
