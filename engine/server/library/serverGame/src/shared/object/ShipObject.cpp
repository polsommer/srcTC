...
bool ShipObject::isVisibleOnClient(Client const &client) const
{
	...
	// Preserve legacy behaviour for space scenes, unloaded ships, and administrative clients
	if (client.isGod() || !isInWorld() || ServerWorld::isSpaceScene())
		return true;

	// Allow ships placed directly in the world cell to be observed like other tangibles on the ground
	if (isInWorldCell())
		return true;

	if (ServerObject const *character = client.getCharacterObject())
	{
		for (ServerObject const *o = character; o; o = safe_cast<ServerObject const *>(ContainerInterface::getContainedByObject(*o)))
			if (o == this)
				return true;
	}
