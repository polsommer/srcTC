...
void CentralServerConnection::onReceive(const Archive::ByteStream &message)
{
    ...
    const std::uint32_t messageType = msg.getType();
    ...
    switch(messageType) {
        ...
        case constcrc("TransferRequestNameValidation"):
        {
            ...
            {
                std::uint32_t const templateCrc = requestNameValidation.getValue().getObjectTemplateCrc();
                if (templateCrc != 0)
                {
        ...
        case constcrc("CtsCompletedForcharacter"):
        {
            ...
            MessageToQueue::getInstance().sendMessageToJava(msg.getValue().second, "ctsCompletedForCharacter", std::vector<int8_t>(), 0, false);
            break;
        }
        case constcrc("GameServerCSRequest"):
        {
            GameServerCSRequestMessage request(ri);
            GameServer::getInstance().handleCSRequest(request);
            break;
        }
        case constcrc("CSFindAuthObject"):
        {
