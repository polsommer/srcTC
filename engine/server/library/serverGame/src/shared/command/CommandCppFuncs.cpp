...
namespace CommandCppFuncsNamespace
{
        ...
        namespace
        {
                char const * const SPACE_LAUNCH_WORLD_LOC_OBJVAR = "space.launch.worldLoc";
                char const * const SPACE_LAUNCH_SHIP_OBJVAR = "space.launch.ship";
                char const * const SPACE_LAUNCH_START_INDEX_OBJVAR = "space.launch.startIndex";
                char const * const SPACE_LAUNCH_REQUEST_TIME_OBJVAR = "space.launch.requestTime";
        }
	...
	CreatureObject * findAndResolveCreatureByNetworkId(NetworkId const & targetId)
	{
        ...
        ShipObject const *getShipFromControlDevice(NetworkId const &shipControlDeviceId)
        {
                ServerObject const * const controlDevice = safe_cast<ServerObject const *>(NetworkIdManager::getObjectById(shipControlDeviceId));
                if (!controlDevice || controlDevice->getGameObjectType() != SharedObjectTemplate::GOT_data_ship_control_device || controlDevice->isBeingDestroyed())
                        return nullptr;

                Container const * const scdContainer = ContainerInterface::getContainer(*controlDevice);
                if (!scdContainer)
                        return nullptr;

                for (ContainerConstIterator iter = scdContainer->begin(); iter != scdContainer->end(); ++iter)
                {
                        ServerObject const * const content = safe_cast<ServerObject const *>((*iter).getObject());
                        if (!content || content->isBeingDestroyed())
                                continue;

                        ShipObject const * const ship = content->asShipObject();
                        if (ship)
                                return ship;
                }
        ...
        bool prepareCreatureForSpaceLaunch(CreatureObject &creature, ShipObject const &ship)
        {
                if (!creature.isAuthoritative())
                {
                        WARNING(true, ("Attempted to prepare non-authoritative creature %s for space launch", creature.getNetworkId().getValueString().c_str()));
                        return false;
                }

                DynamicVariableLocationData const loc(creature.getPosition_w(), ServerWorld::getSceneId(), NetworkId::cms_invalid);
                creature.setObjVarItem(SPACE_LAUNCH_WORLD_LOC_OBJVAR, loc);
                creature.setObjVarItem(SPACE_LAUNCH_SHIP_OBJVAR, ship.getNetworkId());
                creature.setObjVarItem(SPACE_LAUNCH_START_INDEX_OBJVAR, static_cast<int>(0));
                creature.setObjVarItem(SPACE_LAUNCH_REQUEST_TIME_OBJVAR, static_cast<int>(::time(nullptr)));
...
static void commandFuncLaunchIntoSpace(Command const &, NetworkId const & actor, NetworkId const & target, Unicode::String const & params)
{
	...
	if (tokens.empty())
	{
        ...
        ServerObject * const shipControlDevice = safe_cast<ServerObject *>(NetworkIdManager::getObjectById(shipPCDId));
        if (!shipControlDevice || shipControlDevice->getGameObjectType() != SharedObjectTemplate::GOT_data_ship_control_device)
        {
                Chat::sendSystemMessage(*actorCreature, Unicode::narrowToWide("(unlocalized) Invalid ship control device."), Unicode::emptyString);
                return;
        }

        if (shipControlDevice->isBeingDestroyed())
        {
                Chat::sendSystemMessage(*actorCreature, Unicode::narrowToWide("(unlocalized) Ship control device is busy."), Unicode::emptyString);
                return;
        }

        bool shipInDatapad = false;
        ServerObject const *containerWalker = shipControlDevice;
        while (containerWalker)
        {
                containerWalker = safe_cast<ServerObject const *>(ContainerInterface::getContainedByObject(*containerWalker));
                if (containerWalker == actorCreature->getDatapad())
                {
                        shipInDatapad = true;
                        break;
                }
        ...
        if (!shipInDatapad)
        {
                Chat::sendSystemMessage(*actorCreature, Unicode::narrowToWide("(unlocalized) Ship must be in your datapad to launch."), Unicode::emptyString);
                return;
        }

        ShipObject const * const shipToLaunch = getShipFromControlDevice(shipPCDId);
        if (!shipToLaunch)
        {
                Chat::sendSystemMessage(*actorCreature, Unicode::narrowToWide("(unlocalized) Unable to find ship to launch."), Unicode::emptyString);
                return;
        }

        if (shipToLaunch->isInWorld())
        {
                Chat::sendSystemMessage(*actorCreature, Unicode::narrowToWide("(unlocalized) Ship is already active."), Unicode::emptyString);
                return;
        }

        if (shipToLaunch->getOwnerId() != actorCreature->getNetworkId())
        {
                Chat::sendSystemMessage(*actorCreature, Unicode::narrowToWide("(unlocalized) You can only launch ships you own."), Unicode::emptyString);
                return;
        }
		...
		if (tokens.size() > tokenIndex)
		{
			...
			std::replace(strPointLocationResult.begin(), strPointLocationResult.end(), '_', ' ');
                ...
                std::vector<CreatureObject *> launchingCreatures;
                launchingCreatures.reserve(1 + networkIds.size());
                launchingCreatures.push_back(actorCreature);

                for (std::vector<NetworkId>::const_iterator iter = networkIds.begin(); iter != networkIds.end(); ++iter)
                {
                        if (!iter->isValid() || (*iter == actorCreature->getNetworkId()))
                                continue;

                        CreatureObject * const participant = findAndResolveCreatureByNetworkId(*iter);
                        if (!participant)
                        {
                                Chat::sendSystemMessage(*actorCreature, Unicode::narrowToWide("(unlocalized) Unable to locate launch participant."), Unicode::emptyString);
                                return;
                        }

                        if (!participant->getClient())
                        {
                                Chat::sendSystemMessage(*actorCreature, Unicode::narrowToWide("(unlocalized) Launch participant is not connected."), Unicode::emptyString);
                                return;
                        }

                        launchingCreatures.push_back(participant);
                }

                for (std::vector<CreatureObject *>::const_iterator iter = launchingCreatures.begin(); iter != launchingCreatures.end(); ++iter)
                {
                        CreatureObject * const participant = *iter;
                        if (!prepareCreatureForSpaceLaunch(*participant, *shipToLaunch))
                        {
                                Chat::sendSystemMessage(*actorCreature, Unicode::narrowToWide("(unlocalized) Failed to prepare for space launch."), Unicode::emptyString);
                                return;
                        }
                }
