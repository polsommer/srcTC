...
void JavaLibrary::throwScriptException(const char* const format, ...)
{
    va_list args;
    va_start(args, format);
    throwScriptException(format, args);
    va_end(args);
}

void JavaLibrary::throwScriptException(const char* const format, va_list args)
{
    if (!ms_env)
    {
        // Using DEBUG_FATAL assuming it's a macro or function to log fatal errors
        DEBUG_FATAL(!ms_env, ("JavaLibrary::throwScriptException: ms_env is nullptr"));
        return; // Exit early if ms_env is nullptr
    }

    // Use a buffer size that is appropriate for your needs
    constexpr int BUFFER_SIZE = 1524;
    char buffer[BUFFER_SIZE];

    // Print the formatted message to buffer
    vsnprintf(buffer, BUFFER_SIZE, format, args);

    // Ensure buffer is null-terminated
    buffer[BUFFER_SIZE - 1] = '\0';

    // Throw an internal script error with the formatted message
...
// Initial JVM allocation size (minimum)
tempOption.optionString = "-Xms556m";
options.push_back(tempOption);

	// Maximum JVM allocation size
tempOption.optionString = "-Xmx1024m"; // Increase maximum memory allocation
options.push_back(tempOption);

	// Thread stack size per thread
tempOption.optionString = "-Xss1024k"; // Reduce thread stack size to save memory
options.push_back(tempOption);

	// Adjust Metaspace size (for Java 8+)
tempOption.optionString = "-XX:MaxMetaspaceSize=528m"; // Increase Metaspace size
options.push_back(tempOption);

	// JVM performance tuning options
tempOption.optionString = "-Xrs -XX:-UsePerfData -XX:-AllowUserSignalHandlers -XX:UseSSE=4 -XX:+DoEscapeAnalysis -XX:AutoBoxCacheMax=4000 -XX:+OptimizeStringConcat -XX:+OptimizeFill -XX:+EliminateAutoBox -XX:+UseCompressedStrings -XX:+UseCompressedOops -XX:+EliminateLocks -XX:+UseFastAccessorMethods -XX:+UseStringCache -XX:GCPriority=throughput -XX:KeepAreaRatio=1 -XX:LazyUnlocking -XX:CallProfiling -XX:CompactRatio=1 -XXfullSystemGC -XXaggressive -XXgcThreads:yc=4,con=2,par=4";
options.push_back(tempOption);
...
{
    // Ensure the environment is not null
    NOT_NULL(env);
    ...
    if (objId == 0)
    {
        snprintf(buffer, sizeof(buffer), "%s: nullptr object id from script.", errorDescription);
        throwInternalScriptError(buffer);
        return nullptr;
    }
    ...
    {
        snprintf(buffer, sizeof(buffer), "%s: invalid or non-convertible network id [%s].", errorDescription, networkId.getValueString().c_str());
        throwInternalScriptError(buffer);
        return nullptr;
    }

    // Retrieve the object by network ID
    Object *object = NetworkIdManager::getObjectById(networkId);
    if (!object)
    {
        if (!throwIfNotOnServer)
        {
            // Object is not on the server, and caller opted not to throw
            return nullptr;
        }

        snprintf(buffer, sizeof(buffer), "%s: object for network id [%s] not on this server.", errorDescription, networkId.getValueString().c_str());
        throwInternalScriptError(buffer);
        return nullptr;
    }

    // Cast to ServerObject and then to CreatureObject
    ServerObject *serverObject = object->asServerObject();
    if (!serverObject)
    {
        snprintf(buffer, sizeof(buffer), "%s: object id=[%s], template=[%s] is not a ServerObject.", errorDescription, networkId.getValueString().c_str(), object->getObjectTemplateName());
        throwInternalScriptError(buffer);
        return nullptr;
    }

    CreatureObject *creatureObject = serverObject->asCreatureObject();
    if (!creatureObject)
    {
        snprintf(buffer, sizeof(buffer), "%s: object id=[%s], template=[%s] is not a CreatureObject.", errorDescription, networkId.getValueString().c_str(), object->getObjectTemplateName());
        throwInternalScriptError(buffer);
        return nullptr;
    }
...
ShipObject *JavaLibrary::getShipThrow(JNIEnv *env, jlong objId, char const *errorDescription, bool throwIfNotOnServer)
{
    ...
    // Cast to ServerObject and then to ShipObject
    ServerObject *serverObject = object->asServerObject();
    if (!serverObject)
    {
        snprintf(buffer, sizeof(buffer), "%s: object id=[%s], template=[%s] is not a ServerObject.", errorDescription, networkId.getValueString().c_str(), object->getObjectTemplateName());
    ...
    ShipObject *shipObject = serverObject->asShipObject();
    if (!shipObject)
    {
        snprintf(buffer, sizeof(buffer), "%s: object id=[%s], template=[%s] is not a ShipObject.", errorDescription, networkId.getValueString().c_str(), object->getObjectTemplateName());
        throwInternalScriptError(buffer);
        return nullptr;
    }
