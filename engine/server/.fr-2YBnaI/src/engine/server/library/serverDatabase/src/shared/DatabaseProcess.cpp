...
void DatabaseProcess::receiveMessage(const MessageDispatch::Emitter & source, const MessageDispatch::MessageBase & message)
{
    ...
    switch (messageType) {
        ...
        case constcrc("TaskConnectionOpened"):
        {
            ...
            std::string clusterName = ConfigFile::getSection("TaskManager")->findKey("clusterName")->getAsString(0, "");
        ...
        case constcrc("CommoditiesConnectionOpened"):
        {
            ...
            commoditiesConnection = const_cast<CommoditiesServerConnection *>(static_cast<const CommoditiesServerConnection *>(&source));
            break;
        }
        case constcrc("CentralConnectionClosed"):
        {
            ...
            centralServerConnection = nullptr;
        ...
        case constcrc("CommoditiesConnectionClosed"):
        {
            ...
            commoditiesService = nullptr;
            commoditiesConnection = nullptr;
            break;
        }
...
void DatabaseProcess::sendToGameServer(uint32 serverId, const GameNetworkMessage &message)
{
    auto it = gameServerConnections.find(serverId);
    if (it != gameServerConnections.end()) {
        it->second->send(message, true);
    } else {
...
void DatabaseProcess::sendToAnyGameServer(const GameNetworkMessage &message)
{
...
{
    float rateMultiplier = static_cast<float>(ConfigServerDatabase::getQueryReportingRate()) / 60.0f;
    return static_cast<float>(m_queryExecCount) * rateMultiplier;
}
// ----------------------------------------------------------------------
float DatabaseProcess::getQueryFetchRate() const
{
    float rateMultiplier = static_cast<float>(ConfigServerDatabase::getQueryReportingRate()) / 60.0f;
    return static_cast<float>(m_queryFetchCount) * rateMultiplier;
}
