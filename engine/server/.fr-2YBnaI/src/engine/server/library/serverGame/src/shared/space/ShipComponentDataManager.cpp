...
ShipComponentData * ShipComponentDataManager::create (TangibleObject const & component)
{
	...
	{
		WARNING (false, ("ShipComponentDataManager [%s] [%s] is not a component", component.getNetworkId ().getValueString ().c_str (), component.getObjectTemplateName()));
		return nullptr;
	}
...
ShipComponentData * ShipComponentDataManager::create (ShipComponentDescriptor const & shipComponentDescriptor)
{
	...
	case ShipComponentType::SCT_modification:
			...
			break;
	default:
		WARNING (false, ("ShipComponentDataManager::create descriptor has type [%d] invalid", shipComponentDescriptor.getComponentType ()));
		return nullptr;
	}
