...
void ConfigServerGame::install(void)
{
	...
	KEY_INT		(taskManagerPort, 30001);
	KEY_STRING	(centralServerAddress, "localhost");
	KEY_INT		(centralServerPort, 33351);
...
#error unsupported platform
	...
	KEY_FLOAT   (defaultHealthRegen, 20.0f);
	KEY_FLOAT   (defaultActionRegen, 10.0f);
	KEY_FLOAT   (defaultMindRegen, 5.0f);
	KEY_FLOAT   (regenThreshold, 0.5f);
	KEY_FLOAT   (minForcePowerRegenRate, 1.0f);
	KEY_BOOL    (enableAttribModWarnings, false);
	...
	KEY_BOOL    (adminPersistAllCreates, true);
	...
	KEY_INT     (databasePositionUpdateLongDelayIntervalMs, 3*60*100);
	KEY_INT     (databasePositionUpdateShortDelayIntervalMs, 1*100);
	...
	KEY_INT     (maxPlayerSanityCheckFailures, 1);
	KEY_BOOL    (disablePlayerSanityChecker, true);
	...
	KEY_BOOL    (allowMasterObjectCreation, false);
	...
	KEY_BOOL    (buildoutAreaEditingEnabled, true);
