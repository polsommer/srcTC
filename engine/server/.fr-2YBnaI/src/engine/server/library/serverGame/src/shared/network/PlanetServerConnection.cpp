...
void PlanetServerConnection::onConnectionOpened()
{
    ...
    // send process id
    GameGameServerConnect c(GameServer::getInstance().getProcessId(), false, ConfigServerUtility::getSpawnCookie(), static_cast<uint16>(ConfigServerGame::getPreloadNumber()));
    send(c, true);
